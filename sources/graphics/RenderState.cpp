...
namespace hpl {
	...
	int iRenderState::Compare(const iRenderState* apState)  const
	{
		...
		case eRenderStateType_Program:			return CompareProg(apState);
	...
	void iRenderState::SetMode(cRenderSettings* apSettings)
	{
		...
		case eRenderStateType_Program:			SetProgMode(apSettings); break;
	...
	void iRenderState::Set(const iRenderState* apState)
	{
		...
		case eRenderStateType_Program:			mpProgram = apState->mpProgram;
	...
	void iRenderState::SetProgMode(cRenderSettings* apSettings)
	{
		if(mpProgram != apSettings->mpProgram)
		{
			if(apSettings->mbLog){
				if(mpProgram)
					Log("Setting program: '%s'/%d ", mpProgram->GetName().c_str(),
																		(size_t)mpProgram);
				else
					Log("Setting program: NULL ");
			}
			...
			if(mpProgram)
			{
				...
				//reset this so all matrix setting are set to program.
	...
	void iRenderState::SetMatrixMode(cRenderSettings* apSettings)
	{
		...
		if(apSettings->mpProgram)
		{
			...
			apSettings->mpProgram->SetMatrixf("worldViewProj",
	...
	int iRenderState::CompareProg(const iRenderState* apState) const
	{
