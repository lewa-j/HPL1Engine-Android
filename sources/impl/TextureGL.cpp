...
namespace hpl
{
	...
	cTextureGL::cTextureGL(const tString &asName, iPixelFormat *apPxlFmt, iLowLevelGraphics *apLowLevelGraphics,
	...
	bool cTextureGL::CreateFromBitmap(iBitmap2D *pBmp)
	{
		//Generate handles
	...
	bool cTextureGL::CreateAnimFromBitmapVec(tBitmap2DVec *avBitmaps)
	{
	...
	bool cTextureGL::CreateCubeFromBitmapVec(tBitmap2DVec *avBitmaps)
	{
	...
	bool cTextureGL::CreateFromArray(unsigned char *apPixelData, int alChannels, const cVector3l &avSize)
	{
		if (mvTextureHandles.empty())
		{
			mvTextureHandles.resize(1);
	...
	void cTextureGL::SetPixels2D(int alLevel, const cVector2l &avOffset, const cVector2l &avSize,
	...
	bool cTextureGL::CreateFromBitmapToHandle(iBitmap2D *pBmp, int alHandleIdx)
	{
		...
		GetSettings(pBmp, lChannels, internalFormat, format);
		...
		glTexImage2D(GLTarget, 0, internalFormat, mlWidth, mlHeight,
	...
	GLenum cTextureGL::InitCreation(int alHandleIdx)
	{
		GLenum GLTarget = GetGLTextureTargetEnum(mTarget);
	...
	void cTextureGL::PostCreation(GLenum aGLTarget)
	{
		if (mbUseMipMaps && mTarget != eTextureTarget_Rect)
		{
			if (mFilter == eTextureFilter_Bilinear)
